...
export default class Index extends Component<IProps, PageState> {
  ...
  onSubmit = async (e) => {
    ...
    this.addCard({
      name: username,//char # 姓名
      number: number,// char # 银行卡号
      phone: phone,// char # 手机号
      identification: idcard,// char # 身份证
      code: code,// char # 验证码
    })
  }
  ...
  addCard = async (data) => {
    ...
    const {status} = await config.addBank.api.addBank.service({bankcard_info: JSON.stringify(data)})
    if (status === 'ok') {
      Taro.navigateBack()
    }
    Taro.hideLoading()
    ...
    return (
      <View className={styles.index}>
        <Form className={styles.formWrapper} onSubmit={this.onSubmit.bind(this)}>
          ...
          <View className={styles.formItem}>
            <Input className={styles.input} name='phone' type='number' id='phone' onInput={this.onInput.bind(this)}
